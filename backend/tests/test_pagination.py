...
class TestSecretListPaging:
    ...
    def test_grant_list_is_paged(self, client, user1, db_session):
        ...
        rest = client.get(
            f"/secrets/{secret.id}/access?offset=100&limit=200", headers=auth_header(token)
        )
...
class TestWorkflowListPaging:
    def _workflow(self, db_session, owner, *, status="pending", signer=None, recipient=None):
        secret = models.Secret(
            owner_address=owner, name="wf-secret", type="note", encrypted_data="ab" * 8
        )
        db_session.add(secret)
        db_session.flush()
        db_session.add(
            models.AccessGrant(secret_id=secret.id, grantee_address=owner, encrypted_key="00ff")
        )
    ...
    def test_paging_does_not_widen_who_can_see_what(self, client, user1, user2, db_session):
        ...
        visible = {
            ...
            self._workflow(
                db_session, owner["address"], status="completed", recipient=other["address"]
            ).id,
        }
        hidden = {
        ...
        seen |= set(
            _ids(client.get("/multisig/workflows?limit=1&offset=1", headers=auth_header(token2)))
        )
...
class TestGroupListPaging:
    ...
    def test_groups_are_paged_in_activity_order(self, client, user1, db_session):
        ...
        ids = [
            self._group(
                db_session, [user["address"]], last_message_at=base - timedelta(hours=MANY - i)
            )
            for i in range(MANY)
        ]
        ...
        first = [
            row["channel"]["id"] for row in client.get("/groups", headers=auth_header(token)).json()
        ]
        second = [
            row["channel"]["id"]
            for row in client.get("/groups?offset=50", headers=auth_header(token)).json()
        ]
    ...
    def test_a_group_with_no_messages_sorts_on_its_creation(self, client, user1, db_session):
        ...
        token, user = user1
        base = datetime.now(UTC).replace(tzinfo=None)
        old = self._group(db_session, [user["address"]], last_message_at=base - timedelta(days=2))
        ...
        for i in range(MANY):
            ...
            db_session.add(
                models.Message(
                    ...
                    content="ab",
                    created_at=base - timedelta(minutes=MANY - i),
                )
            )
