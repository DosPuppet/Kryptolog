...
class TestRecipientKeyRelease:
    ...
    def test_recipient_keys_accepted_on_final_sign(self, client, user1, user2, recipient_user):
        token1, _ = user1
        token2, u2 = user2
        wf_id = _create_workflow(client, token1, [u2["address"]], [RECIPIENT_ADDRESS]).json()["id"]
...
class TestRecipientAddressNormalization:
    ...
    def test_creation_accepts_a_mixed_case_recipient_key(
        self, client, user1, user2, recipient_user
    ):
        token1, u1 = user1
        _, u2 = user2
        resp = client.post(
        ...
        wf = client.get(
            f"/multisig/workflow/{resp.json()['id']}", headers=auth_header(token1)
        ).json()
        rec = next(r for r in wf["recipients"] if r["user_address"] == RECIPIENT_ADDRESS)
        assert rec["encrypted_key"] == "creation_key"

    def test_completing_signature_accepts_a_mixed_case_recipient_key(
        self, client, user1, user2, recipient_user
    ):
        token1, _ = user1
        token2, u2 = user2
    ...
    def test_refuses_to_complete_when_a_recipient_would_hold_no_key(
        self, client, user1, user2, recipient_user
    ):
    ...
    def test_refuses_keys_for_someone_who_is_not_a_recipient(
        self, client, user1, user2, recipient_user
    ):
        token1, u1 = user1
        token2, u2 = user2
...
class TestThreshold:
    ...
    def test_completing_signer_may_release_under_threshold(
        self, client, user1, user2, user3, recipient_user
    ):
        ...
        r1 = client.post(
            ...
            json={
                "signature": "s1",
                "recipient_keys": {RECIPIENT_ADDRESS: "bad"},
            },
            headers=auth_header(token1),
        )
...
class TestRejectWorkflow:
    def test_signer_can_reject(self, client, user1, user2):
        ...
        resp = client.post(
            f"/multisig/workflow/{wf_id}/reject",
            json={"reason": "nope"},
            headers=auth_header(token2),
        )
        assert resp.status_code == 200
    ...
    def test_non_signer_cannot_reject(self, client, user1, user2):
        ...
        wf_id = _create_workflow(client, token1, [u2["address"]]).json()["id"]
        resp = client.post(
            f"/multisig/workflow/{wf_id}/reject", json={}, headers=auth_header(token1)
        )
        assert resp.status_code == 403
...
class TestDeleteWorkflow:
    def test_owner_deletes_rejected_workflow(self, client, user1, user2):
        ...
        # Workflow is gone.
        assert (
            client.get(f"/multisig/workflow/{wf_id}", headers=auth_header(token1)).status_code
            == 404
        )
