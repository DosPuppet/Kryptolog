...
const MultisigList = ({ workflows, onSelect, onCreate, loading }) => {
    ...
    const statusDot = (status) => {
        const color = status === 'completed' ? 'bg-emerald-500' : (status === 'failed' || status === 'rejected') ? 'bg-red-500' : 'bg-amber-500';
        ...
        <div>
            ...
            <div className={layoutClass}>
                {sortedWorkflows.map(wf => {
                    ...
                    // N-of-M: measure progress against the threshold (N), not M.
                    const required = wf.threshold || total;
                    const progress = Math.min(100, (completed / required) * 100);
                    const barColor = wf.status === 'completed' ? 'bg-emerald-500' : wf.status === 'rejected' ? 'bg-red-500' : 'bg-amber-500';
                                            ...
                                            <div className="flex items-center gap-2">
                                                {wf.status === 'completed' ? (
                                                    ...
                                                    <span className="px-2 py-0.5 bg-red-100 text-red-700 rounded-full text-xs font-bold flex items-center gap-1">
                                                        <AlertTriangle className="w-3 h-3" /> {wf.status === 'rejected' ? 'Rejected' : 'Failed'}
                                                    </span>
                                                ) : (
                        ...
                        >
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between text-xs text-slate-500">
                                    <span>Progress</span>
                                    <span>{completed}/{required} required{required !== total ? ` (${total} signers)` : ''}</span>
                                </div>
