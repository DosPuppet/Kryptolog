...
import { X, Plus, Trash2, Upload, User, RefreshCw, Timer, Share2 } from 'lucide-react';
import { vaultService } from '../services/vault';
import { confirmDialog } from '../utils/confirm';
import SendKeysModal from './transfer/SendKeysModal';
...
export default function VaultManager({ onClose }) {
    ...
    // Load a backup file into the import view. Accepts a plaintext JSON export or
    // an encrypted .kvault backup; the latter reveals a passphrase field.
    const handleImportFile = (e) => {
        ...
        reader.onload = (ev) => {
            ...
            try {
                ...
                setImportIsEncrypted(!!(data && data.salt && data.iv && data.data && !data.accounts));
            } catch {
                setImportIsEncrypted(false);
            }
        };
        reader.onerror = () => setMsg({ type: 'error', text: 'Could not read the selected file.' });
        reader.readAsText(file);
    };

    const handleImport = async () => {
        try {
            const count = await importVault(importJson, importIsEncrypted ? importPass : undefined);
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-850 w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col max-h-[80vh] mx-4">
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    ...
                    {view === 'import' && (
                        <div className="space-y-4">
                            <h3 className="font-semibold dark:text-white">Import Vault</h3>
                            <p className="text-xs text-slate-500">
                                Choose an encrypted <span className="font-mono">.kvault</span> backup or a plain JSON
                                export, or paste JSON below. Accounts are merged into your vault.
                            </p>
                            ...
                            <label className="w-full flex items-center gap-2 cursor-pointer bg-slate-50 dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-600 dark:text-slate-300 hover:border-emerald-500 transition-colors">
                                ...
                                <span className="truncate">{importFileName || 'Choose a .kvault or .json file…'}</span>
                                <input type="file" accept=".kvault,.json,application/json" className="hidden" onChange={handleImportFile} />
                            </label>
                            ...
                            <textarea
                                ...
                                placeholder="…or paste JSON / .kvault content here"
                                value={importJson}
                                onChange={e => {
                            ...
                            <div className="flex gap-2">
                                <button onClick={handleImport} disabled={!importJson} className="flex-1 bg-emerald-600 text-white py-2 rounded-lg font-medium hover:bg-emerald-500 disabled:opacity-50">Import</button>
                                <button onClick={() => setView('list')} className="flex-1 bg-slate-200 dark:bg-slate-750 text-slate-700 dark:text-slate-300 py-2 rounded-lg font-medium">Cancel</button>
                            </div>
                        </div>
                    )}
                ...
                <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-850/50 rounded-b-2xl flex justify-between gap-2">
                    {view === 'list' && (
                        <>
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setShowSend(true)} className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-slate-750 border border-slate-200 dark:border-slate-600 rounded-lg text-sm hover:border-emerald-500 transition-colors dark:text-white" title="Transfer or back up keys">
                                    <Share2 className="w-4 h-4 text-emerald-500" /> Transfer / Back up
                                </button>
                                <button onClick={() => setView('import')} className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-slate-750 border border-slate-200 dark:border-slate-600 rounded-lg text-sm hover:border-indigo-500 transition-colors dark:text-white" title="Import vault (.kvault or JSON)">
            ...
            {showSend && <SendKeysModal onClose={() => setShowSend(false)} />}
        </div>
    );
}
