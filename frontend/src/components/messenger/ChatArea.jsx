    ...
    return (
        <div className="flex flex-1 flex-col bg-slate-50 dark:bg-slate-900 h-full">
            ...
            <div className="p-4 bg-white dark:bg-slate-850 border-b border-slate-200 dark:border-slate-700 flex items-center gap-3 shadow-sm z-10">
                <button
                    onClick={onBack}
                    className="md:hidden p-2 -ml-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full"
                >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {loadingMessages ? (
                    ...
                    (() => {
                    // F-2: if the peer has ever produced a valid signature in this
                    // conversation, a later unsigned message is suspicious (likely
                    // signature-stripping), not a benign legacy message.
                    const hasPeerSignedHistory = activeConversation.messages.some(
                            ...
                            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                ...
                                `}>
                                    ...
                                    <div className={`flex items-center gap-1 text-[10px] mt-1 opacity-70 ${isMe ? 'text-indigo-100' : 'text-slate-400'}`}>
                                        ...
                                        {/* Authenticity (audit S1/F-2): verified / invalid / unsigned. */}
                                        {!isMe && (
