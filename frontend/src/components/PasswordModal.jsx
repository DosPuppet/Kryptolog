...
const PasswordModal = ({ isOpen, onSubmit, onCancel, message, onBiometric }) => {
    ...
    // Clear the field each time the modal transitions to open. This is a
    // render-time reset (React's endorsed pattern for adjusting state on a prop
    // change) rather than a setState-in-effect.
    if (isOpen && !wasOpen) {
        setWasOpen(true);
        setPassword('');
